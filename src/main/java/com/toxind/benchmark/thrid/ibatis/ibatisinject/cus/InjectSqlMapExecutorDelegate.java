...
import com.toxind.benchmark.thrid.ibatis.ibatisinject.IbatisTest;
@SuppressWarnings("deprecation")
public class InjectSqlMapExecutorDelegate extends SqlMapExecutorDelegate {
	private SqlMapExecutorDelegate delegate = null;
	private InjectSqlMapClient client = null;
	
	
	public void setClient(InjectSqlMapClient client) {
		this.client = client;
	}
	public InjectSqlMapExecutorDelegate(SqlMapExecutorDelegate delegate) {
		super();
		this.delegate = delegate;
	}
	protected synchronized void checkAndRefreshSqlMap(String id) {
		String testId = IbatisTest.map.get("id");
		String namespace = testId.substring(0,testId.indexOf('.'));
		String subId = testId.substring(testId.indexOf('.') + 1);
		...
		mappingStr.append("<sqlMap namespace=\"").append(namespace).append("\">").append("\r\n");
		mappingStr.append("<select id=\"").append(subId).append("\" parameterClass=\"map\" resultClass=\"").append(IbatisTest.map.get("resultClass")).append("\">").append("\r\n");
//		mappingStr.append("<![CDATA[").append("\r\n");
		mappingStr.append(IbatisTest.map.get("sqlContent")).append("\r\n");
//		mappingStr.append("]]>").append("\r\n");
